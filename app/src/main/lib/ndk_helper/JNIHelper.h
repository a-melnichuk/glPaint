...
#define PATH_CAT(b) "melnichuk/al/coloringbookapp/utils/NDKHelper" b
...
class JNIHelper {
 ...
 public:
  ...
  /*
   * Load and create OpenGL maskTexture from given file name.
   * The method invokes BitmapFactory in Java so it can read jpeg/png formatted
   * files
   *
   * The methods creates mip-map and set maskTexture parameters like this,
   ...
   * return:
   * OpenGL maskTexture name when the call succeeded
   * When it failed to load the maskTexture, it returns -1
   */
  uint32_t LoadTexture(const char* file_name, int32_t* outWidth = nullptr,
                       int32_t* outHeight = nullptr, bool* hasAlpha = nullptr);

  /*
   * Load and create OpenGL cubemap maskTexture from given file name
   ...
   */
  void LoadImageBuffer(const char* file_name,
                       int32_t* outWidth,
                       int32_t* outHeight,
                       bool* hasAlpha,
                       void* outAddress,
