...
//#define NATIVE_ACTIVITY_JAVA_PATH "com/sample/helper/NDKHelper"
#define NATIVE_ACTIVITY_JAVA_PATH "melnichuk/al/coloringbookapp/utils/NDKHelper"
...
uint32_t JNIHelper::LoadTexture(const char* file_name, int32_t* outWidth,
  ...
  jclass javaCls =
      RetrieveClass(env, PATH_CAT("$TextureInformation"));
  ...
  LOGI("Loaded maskTexture original size:%dx%d alpha:%d", width, height,
       (int32_t)alpha);
  if (outWidth != NULL) {
    *outWidth = width;
  }
...
void JNIHelper::LoadImageBuffer(const char* file_name, int32_t* outWidth,
                             int32_t* outHeight, bool* hasAlpha, void* outAddress, jlong* outSize)
{
  if (activity_ == NULL) {
    LOGI(
            "JNIHelper has not been initialized. Call init() to initialize the "
  ...
  outAddress = env->GetDirectBufferAddress(objGlobal);
  if (outSize != NULL)
    *outSize = env->GetDirectBufferCapacity(objGlobal);
}
